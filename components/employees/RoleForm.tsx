...
> = {
  ...
  inventory:    { header: 'bg-yellow-50 text-yellow-900', bullet: 'bg-yellow-500' },
...
export function RoleFormSheet({ open, value: role, onClose }: Props) {
  ...
  const toggleSectionAll = (sectionKey: keyof RolePermissions, next: boolean) => {
    ...
    <Sheet open={open} onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="flex w-full flex-col sm:max-w-3xl">
        <SheetHeader>
          ...
          <SheetDescription>
            Permission flags are stored but not yet enforced. Turn them on
            now so the data is ready when the gate lands.
          </SheetDescription>
        </SheetHeader>
                  ...
                  <div key={section.key} className="overflow-hidden rounded-lg border">
                    ...
                    >
                      <span className="inline-flex items-center gap-2">
                        <span className={`inline-block size-2 rounded-full ${styles.bullet}`} />
                        {section.label}
                    ...
                    <div className="grid grid-cols-1 gap-x-4 gap-y-1.5 p-3 sm:grid-cols-2">
                      {section.flags.map((flag) => (
                        <Controller
                          key={flag.key}
                          control={form.control}
                          name={`permissions.${section.key}.${flag.key}` as never}
                          render={({ field }) => (
                            <label className="flex items-center gap-2 text-sm">
                              <input
                                type="checkbox"
