...
export async function getPosition(ctx: Context, id: string): Promise<Position> {
  const { data, error } = await ctx.db.from('positions').select('*').eq('id', id).single()
  if (error || !data) throw new NotFoundError(`Position ${id} not found`)
  return data
}

export async function createPosition(ctx: Context, input: unknown): Promise<Position> {
  ...
  const { data, error } = await ctx.db
    ...
    .single()
  if (error) {
    if (error.code === '23505') throw new ConflictError('A position with that name already exists')
...
export async function deactivatePosition(ctx: Context, id: string): Promise<Position> {
  const { data, error } = await ctx.db
    .from('positions')
