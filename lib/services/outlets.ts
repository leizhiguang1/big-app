...
export async function listOutlets(ctx: Context): Promise<OutletWithRoomCount[]> {
...
export async function createOutlet(ctx: Context, input: unknown): Promise<Outlet> {
...
export async function deactivateOutlet(ctx: Context, id: string): Promise<Outlet> {
  const { data, error } = await ctx.db
    .from('outlets')
...
export async function listRooms(ctx: Context, outletId: string): Promise<Room[]> {
  const { data, error } = await ctx.db
    .from('rooms')
    .select('*')
...
export async function createRoom(
  ...
  if (error) {
    if (error.code === '23505')
      throw new ConflictError('A room with that name already exists at this outlet')
    throw new ValidationError(error.message)
  }
  return data
}
