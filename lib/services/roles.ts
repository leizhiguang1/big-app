...
export async function getRole(ctx: Context, id: string): Promise<Role> {
  const { data, error } = await ctx.db.from('roles').select('*').eq('id', id).single()
...
export async function createRole(ctx: Context, input: unknown): Promise<Role> {
  ...
  const { data, error } = await ctx.db
    ...
    .single()
  if (error) {
    if (error.code === '23505') throw new ConflictError('A role with that name already exists')
    throw new ValidationError(error.message)
  }
  return hydrate(data)
}

export async function updateRole(ctx: Context, id: string, input: unknown): Promise<Role> {
  const parsed = roleInputSchema.parse(input)
  const { data, error } = await ctx.db
    .from('roles')
    .update({
...
export async function deactivateRole(ctx: Context, id: string): Promise<Role> {
  const { data, error } = await ctx.db
    .from('roles')
