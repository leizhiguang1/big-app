...
| **3 — Messaging stack** | Layered on top of Phase 1 + 2 clinic core. Three sibling modules that evolve independently: Conversations (channel-agnostic inbox, WhatsApp v1 provider via wa-connector), CRM (tags/notes/tasks), Automations (hard-coded trigger→action). AI assistant deferred. See [ARCHITECTURE.md §3a](./ARCHITECTURE.md). | [11 Conversations](./modules/11-conversations.md) · [13 CRM](./modules/13-crm.md) · [14 Automations](./modules/14-automations.md) |
...
23. **Cross-service communication = HTTP + HMAC webhooks only (2026-04-20).** big-app never reads wa-connector's tables directly and never uses Supabase Realtime for service-to-service events. Realtime stays as a browser ↔ DB tool only. This is the load-bearing rule that keeps the two services extractable and the "one Supabase project today, separate projects later" decision cheap. See [ARCHITECTURE.md §2.1](./ARCHITECTURE.md).
...
_None blocking Phase 1 build. wa-connector's internal schema shape is not tracked here — it lives in the wa-connector repo. big-app's only obligation is to never touch `wa_*` tables._
